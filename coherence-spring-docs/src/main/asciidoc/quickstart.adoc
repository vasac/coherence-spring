...
an exposed REST API. The data is saved in an embedded http://hsqldb.org/[HSQL database]. The caching is implemented at
the service layer.
...
[source,shell,indent=1,subs="verbatim,quotes,attributes"]
----
$ java -jar samples/coherence-spring-demo/coherence-spring-demo-boot/target/coherence-spring-demo-boot-{coherence-spring-version}.jar
----
...
We see that an Event with the id `1` was successfully created. Let's verify that the _cache put_ worked by inspecting
the cache using the open-source tool https://visualvm.github.io/[VisualVM].

.VisualVM - Cache Put
image::visual-vm-cache-put.png[VisuamVM]

Under the `MBeans` tab you will find the Cache MBeans, including and entry for the `events` cache,
...
TIP: When using VisualVM consider installing the respective
https://medium.com/oracle-coherence/open-source-coherence-visualvm-plugin-released-533ea2f576bc[Coherence VisualVM Plugin]
...
Let's check the statistics again, this time via the Coherence VisualVM Plugin:

.Cache Statistics via Coherence VisualVM Plugin
image::visual-vm-cache-get-value.png[VisuamVM Get Value From Cache]
...
<2> Add the desired version of Coherence
