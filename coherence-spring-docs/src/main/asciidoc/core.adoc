...
with the open source Coherence CE. Therefore, we don't bring in
...
IMPORTANT: Coherence Spring requires as a minimum version *Coherence CE* `22.06`.

[[coherence-spring-botstrap]]
== Bootstrapping Coherence

Coherence Spring uses the Coherence
{coherence-docs}develop-applications/starting-and-stopping-cluster-members.html#GUID-A74280C1-AB23-48DB-8C0D-E494230ABFCA[Bootstrap API]
to configure and create Coherence instances. This means that Coherence resources in a Spring application are typically
...
[IMPORTANT]
====
All Coherence resources that are used by Spring application code should be injected using Spring dependency injection, only.
Applications should avoid calling Coherence APIs that create or initialize Coherence resources directly, especially
static {coherence-api}com/tangosol/net/CacheFactory.html[CacheFactory] methods. If your Spring application code calls
these Coherence APIs it may cause Coherence to be initialized too early in the start-up process, before Spring has
initialized the Coherence extensions. A typical symptom of this would be that Coherence starts without picking up the
correct configuration from Spring.
====
...
annotated with `@Lazy`. This is to avoid deadlocks where a cache bean requires another bean to be injected in its
configuration, which will happen on a different thread to the main Spring thread. A consequence of this is that all
cache beans will be Spring lazy dynamic proxies.
====

[[coherence-spring-dependency-injection-type-conversion]]
==== Type Conversion of NamedMap and NamedCache
...
IMPORTANT: When defining your own `ConversionService` bean, please make sure that the `CoherenceGenericConverter` is added to it.
...
[source,java]
----
include::{include-source-code}di/viewtransformer/PeopleService.java[tags=!*]
----
<1> The name of the underlying map for the view is "people".
<2> The `@View` annotation specifies that a view will be injected rather than a raw `NamedMap`.
...
[[injecting-spring-beans-into-coherence-managed-objects]]
== Injecting Spring Beans into Coherence-Managed Objects

Coherence has many server-side extension points, which enable you to customize application behavior in different ways,
...
Spring beans into these Coherence-managed extension points using the custom configuration namespace handler.

[source,xml]
.coherence-cache-config.xml with Spring NamespaceHandler
----
include::{include-source-code-samples-cachestore-resources}coherence-cache-config.xml[tag=header]
----

After you have declared the handler for the Spring namespace above, you can specify the `<spring:bean>` element in any
place where you would normally use the `<class-name>` or `<class-factory-name>` elements:

[source,xml]
.Using Spring bean references in coherence-cache-config.xml
----
<?xml version="1.0"?>
<cache-config xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <interceptors>
        <interceptor>
            <instance>
                <spring:bean>registrationListener</spring:bean> <!--1-->
            </instance>
        </interceptor>
        <interceptor>
            <instance>
    ...
    <caching-scheme-mapping>
        <cache-mapping>
            ...
            <init-params>
                <init-param>
                    <param-name>partitionListener-bean</param-name> <!--2-->
    ...
    <caching-schemes>
        <distributed-scheme>
            ...
            <partition-listener>
                <spring:bean>{partitionListener-bean}</spring:bean> <!--3-->
            </partition-listener>
            <member-listener>
...
----
<1> Specifying the name of the Spring bean that you want to reference.
<2> The name of Spring beans can also be parameterized. Let's create a parameter named `partitionListener-bean` with the name of the Spring bean as its value.
<3> Here we reference the parameter named `partitionListener-bean` and its value will be used.

TIP: Please also take a look at the chapter discussing the
<<coherence-spring-cachestore-demo, Coherence Spring Cachestore Demo>> as it uses Spring-managed beans.
...
[source,java]
----
include::{include-source-code}event/PersonEventHandler.java[tag=!*]
----
...
[source,java]
----
include::{include-source-code}filterbinding/LargePalmTrees.java[tags=!*]
----
...
[source,java]
----
include::{include-source-code}filterbinding/LargePalmTreesFilterFactory.java[tags=!*]
----
...
include::{include-source-code}filterbinding/PalmTrees.java[tags=!*]
----
...
[source,java]
----
include::{include-source-code}filterbinding/PalmTreesFilterFactory.java[tags=!*]
----
...
[source,java]
----
include::{include-source-code}filterbinding/PlantService.java[tags=!*]
----
...
include::{include-source-code}extractorbinding/PlantNameExtractor.java[tags=!*]
----
...
[source,java]
----
include::{include-source-code}extractorbinding/PlantNameExtractorFactory.java[tags=!*]
----
...
include::{include-source-code}extractorbinding/PlantService.java[tags=!*]
----
<1> The `@View` annotation indicates that this is a view rather than a plain `NamedMap`
...
[id=coherence-spring-messaging-with-coherence-topics]
...
{spring-docs-data-jpa}#jpa.repositories[JPA Repositories] make basic CRUD database access very simple. An application
...
* {coherence-api}com/tangosol/net/cache/CacheLoader.html[CacheLoader] - an application developer writes an implementation of a `CacheLoader` to read data from a database for
a given key (or keys), convert it to entities that are then loaded into a cache for the given keys.
* {coherence-api}com/tangosol/net/cache/CacheStore.html[CacheStore] - whilst a `CacheLoader` only loads from a database into a cache, a `CacheStore` (which extends
`CacheLoader`) also stores cached entities back to the database, or for entries deleted from the cache, erases the
corresponding values from the database. The parallels between a `CacheLoader` or `CacheStore` and a `JpaRepository`
should be pretty obvious.

The Coherence Spring core module provides two interfaces:

- {coherence-spring-api}com/oracle/coherence/spring/cachestore/JpaRepositoryCacheLoader.html[JpaRepositoryCacheLoader], which extends both `JpaRepository` and `CacheLoader`
- {coherence-spring-api}com/oracle/coherence/spring/cachestore/JpaRepositoryCacheStore.html[JpaRepositoryCacheStore], which extends both `JpaRepository` and `CacheStore`.
...
NOTE: Please also look at the reference documentation chapter <<injecting-spring-beans-into-coherence-managed-objects>>.

[[coherence-spring-cachestore-demo]]
=== JPA Repository CacheStore Demo
...
* Embedded Coherence
* Connect to a remote Coherence instance cache using
{coherence-docs}develop-remote-clients/introduction-coherenceextend.html[Coherence*Extend]
...
[[coherence-spring-cachestore-demo-model]]
==== Data Model
...
[source,java]
----
include::{include-source-code-samples-cachestore}model/Person.java[tag=person]
----
...
[[coherence-spring-cachestore-demo-jpa-repository]]
...
[source,java]
----
include::{include-source-code-samples-cachestore}model/PersonRepository.java[tag=imports]

include::{include-source-code-samples-cachestore}model/PersonRepository.java[tag=personRepo]
----
...
[[coherence-spring-cachestore-demo-embedded]]
==== Embedded Coherence

In the embedded Coherence CacheStore demo we use a co-located Coherence instance that will start
...
.coherence-cache-config.xml
----
include::{include-source-code-samples-cachestore-resources}coherence-cache-config.xml[tag=scheme]
----
...
.coherence-cache-config.xml
----
include::{include-source-code-samples-cachestore-resources}coherence-cache-config.xml[tag=mapping]
----
...
[NOTE]
====
The bean name used here is `personRepository`. This is the default name generated by Spring for the `PersonRepository`
class, which is the simple class name with the first letter lowercase. If we did not want to rely on Spring generating a
bean name we could specify a name in the `@Repository` annotation on the `PersonRepository` class.
====
...
[[coherence-spring-cachestore-demo-running-embedded]]
==== Running the Embedded Sample
...
The controller class for the two endpoints is very simple:

[source,java]
.PersonController.java
----
include::{include-source-code-samples-cachestore-app}controller/PersonController.java[tag=code]
----
...
----
java -jar samples/cachestore-demo/coherence-spring-cachestore-demo-app/target/coherence-spring-cachestore-demo-{coherence-spring-version}.jar
----
...
curl -i -X POST http://localhost:8080/api/people \
    ...
    -d 'age=21' -d 'id=100'
----
This will create the `Person` named Joe Smith with the id 100. This should return with a 200 response to say the `Person`
...
[[coherence-spring-cachestore-demo-coherence-extend]]
==== Using Coherence*Extend

This is the slightly more complex version of the CacheStore demo. Instead of using an embedded version Coherence, we
will have a remote Coherence instance and the actual application will connect to Coherence via Coherence*Extend.
...
We now have to start the Coherence Server as well as the Coherence Client App. We run both apps using Spring Boot. Let's
start with the Coherence Server:

[source,bash,subs="verbatim,quotes,attributes"]
----
java -jar samples/cachestore-demo/coherence-spring-cachestore-demo-server/target/coherence-spring-cachestore-demo-server-{coherence-spring-version}.jar
----

Next we start the client app. It is actually the same app as used in the embedded Coherence use-case. However, we will
specify an additional Spring Boot profile, instructing the app to connect to the Coherence server in client mode via
Coherence*Extend:

[source,bash,subs="verbatim,quotes,attributes"]
----
java -jar samples/cachestore-demo/coherence-spring-cachestore-demo-app/target/coherence-spring-cachestore-demo-app-{coherence-spring-version}.jar \
--spring.profiles.active=remote
----

By activating the `remote` Spring Boot profile, we will configure Coherence for client mode and we reference a different
Cache Configuration XML file called `remote-cache-config.xml`.

[[coherence-spring-cachestore-demo-inspect-database]]
==== Inspecting the Database

In the remote Coherence CacheStore demo, HSQL will be instantiated via the server module. This allow the app as well as
...
such as the open-source https://dbeaver.io/[DBeaver].
