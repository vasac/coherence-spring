...
- *SERVER* - Join as Coherence cluster member. This is the default dession type.
...
by annotating the injection point with link:{coherence-spring-api}com/oracle/coherence/spring/SessionName.html[@SessionName].
...
A typical stand-alone Coherence application would create a `NamedTopic` along with `Publisher` or `Subscriber` instances to publish to or subscribe to topics. Injection of topics into Spring applications is already covered in <<injecting-namedtopic,Injecting NamedTopics>>. With Spring messaging this becomes much simpler.
...
To create a topic Publisher that sends messages you can simply define an interface that is annotated with link:{coherence-spring-api}com/oracle/coherence/spring/annotation/CoherencePublisher.html[@CoherencePublisher`].
